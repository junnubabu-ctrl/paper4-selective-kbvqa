...
def main():
    ap=argparse.ArgumentParser(description="Run B2/B3 inference path: retrieval + relevance filtering + answer/evidence verification. Calibration/selective policy is fitted separately on validation outputs.")
    ap.add_argument("--manifest",required=True); ap.add_argument("--out",required=True); ap.add_argument("--checkpoint",required=True); ap.add_argument("--run-id",default="proposed")
    ...
    args=ap.parse_args(); state=run(args,"full")
